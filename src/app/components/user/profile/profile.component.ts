...
export class ProfileComponent implements OnInit {
  ...
  constructor(public service:UserService,
    ...
    public dialogRef:MatDialogRef<ProfileComponent>,public sPasswordService:PasswordService,public servicePlanta:PlantasService) { }
...
getCheckbox(checkbox){
 ...
 // then, we make object array of checked, and value by checked variable  
    ...
    return this.plantaUser=checkbox.value
...
// }
  ...
  ngOnInit(): void {
    ...
    var id=obj.primarysid
    this.loadFullDataUser(id)
 ...
 openDialog(value) {
  ...
  /*task: Task = {
    ...
    color: 'primary',
    subtasks: [
      {name: 'Primary', completed: false, color: 'accent'},
  ...
  updateAllComplete() {
    this.allComplete = this.task[1].subtasks != null && this.task[1].subtasks.every(t => t.completed);
  }

  someComplete(): boolean {
    if (this.task[1].subtasks == null) {
      console.log(this.task[1].subtasks)
      return false;
      
    }
    return this.task[1].subtasks.filter(t => t.completed).length > 0 && !this.allComplete;
  }
  ...
  setAll(completed: boolean) {
    ...
    this.task[1].subtasks.forEach(t => t.completed = completed);
  }
